...
Monty can use three types of action output to recognize the sensed objects efficiently. 1) it can move the patch over the full ultrasound image, akin to moving it over the full camera image from the iPad. 2) it can suggest a location in the room for the human operator to move the ultrasound probe to to get a different view. 3) it can adjust the settings of the probe, such as depth of field and gain. None of these are required for object recognition to work, but each one can make recognition more efficient and robust.
