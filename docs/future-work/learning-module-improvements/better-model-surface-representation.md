...
![How we currently account for surface when searching the models. Depending on the amount of curvature sensed, the search radius will be narrowed more or less in the direction of the sensed surface normal. Points in the radius that have a surface normal that points in the opposite direction of the hypothesized one with be ignored in the evidence calculation to account for thin surface where points on both side of a surface will end up in the radius.](../../figures/future-work/search_incl_surface_rep.png)